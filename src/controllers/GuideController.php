...
class GuideController
{
    ...
    private function fetchProfile(PDO $pdo, int $userId): ?array
    {
        ...
        try {
            ...
            // Log error nếu cần
            error_log('Error fetching guide profile: ' . $e->getMessage());
        }

        // Fallback: thử lấy từ guide_files (dữ liệu cũ)
        try {
            $stmt = $pdo->prepare('SELECT * FROM guide_files WHERE user_id = :user_id LIMIT 1');
            $stmt->execute(['user_id' => $userId]);
            $profile = $stmt->fetch();
            return ($profile && is_array($profile)) ? $profile : null;
        } catch (PDOException $e) {
            return null;
        }
    }
    ...
    private function getGuideById(int $id): ?array
    {
        ...
        $this->ensureProfileTable($pdo);
        // Lấy user có role = 'huong_dan_vien' HOẶC có profile trong guide_profiles
        $stmt = $pdo->prepare("
            SELECT u.* 
            FROM users u
            LEFT JOIN guide_profiles gp ON gp.user_id = u.id
            WHERE u.id = :id AND (u.role = :role OR gp.user_id IS NOT NULL)
            LIMIT 1
        ");
    ...
    public function index(): void
    {
        ...
        if ($pdo) {
            try {
                $this->ensureProfileTable($pdo);
                // Lấy tất cả user có role = 'huong_dan_vien' HOẶC có profile trong guide_profiles
                $stmt = $pdo->prepare("
                    SELECT DISTINCT u.*, gp.phone, gp.languages, gp.group_type
                    FROM users u
                    LEFT JOIN guide_profiles gp ON gp.user_id = u.id
                    WHERE u.role = :role OR gp.user_id IS NOT NULL
                    ORDER BY u.id ASC
                ");
                $stmt->execute(['role' => self::ROLE_GUIDE]);
                $result = $stmt->fetchAll();
                $guides = is_array($result) ? $result : [];
            } catch (PDOException $e) {
    ...
    public function edit(): void
    {
        ...
        if ($pdo) {
            $this->ensureProfileTable($pdo);
            $profile = $this->fetchProfile($pdo, $id);
        } else {
            $profile = null;
        }
    ...
    public function update(): void
    {
        ...
        if ($pdo) {
            ...
            $result = $stmt->fetch();
            $oldAvatar = $result ? ($result['avatar'] ?? null) : null;
        }
