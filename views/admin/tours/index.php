            ...
            <?php foreach ($tours as $tour): ?>
              <tr>
                ...
                <?php
                  ...
                  if (is_array($scheduleData) && isset($scheduleData['days']) && is_array($scheduleData['days']) && count($scheduleData['days']) > 0) {
                      $daysCount = count($scheduleData['days']);
                      $firstDate = $scheduleData['days'][0]['date'] ?? null;
                      $lastDate = $scheduleData['days'][$daysCount - 1]['date'] ?? $firstDate;

                      $scheduleDisplay = $daysCount . ' ngày';
                      if ($firstDate) {
                          $scheduleDisplay .= ' (' . $firstDate;
                          if ($lastDate && $lastDate !== $firstDate) {
                              $scheduleDisplay .= ' - ' . $lastDate;
                          }
                          $scheduleDisplay .= ')';
                      }
                  } else {
                ...
                <td class="d-flex justify-content-center gap-2">
                  ...
                  <form action="<?= BASE_URL ?>tour-delete" method="POST" onsubmit="return confirm('Xác nhận xóa tour?');">
